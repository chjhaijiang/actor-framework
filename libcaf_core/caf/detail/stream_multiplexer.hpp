...
public:
  ...
  /// Stores previous and next stage for a stream.
  struct stream_state {
  ...
  stream_multiplexer(local_actor* self, backend& service, actor& basp);
...
protected:
  // Prepares this handler for invoking the content of `x` by setting
  // `current_stream_msg_`.
  void prepare_invoke(stream_msg& x);
  ...
  optional<remote_path&> get_remote(const node_id& nid);
  ...
  /// @pre `current_remote_path_ != remotes_.end()`
  void manage_credit();
  ...
  inline void send_remote_ctrl(remote_path& path, message&& x) {
    basp_->enqueue(make_basp_message(path, std::move(x)), self_->context());
  }
  ...
  /// Stores which remote path belongs to `current_stream_msg_`.
  remote_paths::iterator current_remote_path_;
