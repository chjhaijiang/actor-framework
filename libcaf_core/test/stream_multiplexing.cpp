...
behavior dummy_basp(event_based_actor* self) {
  return {
...
printf("%s %d - %s\n", __FILE__, __LINE__,
       to_string(self->current_mailbox_element()->content()).c_str());
...
/// Forwards control messages that flow upstream to `ControlHandler` and data
/// messages that flow downstream to `DataHandler`. Error messages are forwarded
/// to both handlers.
template <class ControlHandler, class DataHandler>
class stream_msg_splitter {
public:
  using result_type = void;

  stream_msg_splitter(stream_msg& x, ControlHandler& f, DataHandler& g)
      : x_(x),
        f_(f),
        g_(g) {
    // nop
  ...
  template <class T>
  typename std::enable_if<T::label == stream_msg::flows_downstream>::type
  operator()(T& y) {
    g_(x_, y);
  }
  ...
  template <class T>
  typename std::enable_if<T::label == stream_msg::flows_upstream>::type
  operator()(T& y) {
    f_(x_, y);
  }

  template <class T>
  typename std::enable_if<T::label == stream_msg::flows_both_ways>::type
  operator()(T& y) {
    f_(x_, y);
    g_(x_, y);
  }
...
template <class ControlHandler, class DataHandler>
void split_stream_msg(stream_msg& x, ControlHandler& f, DataHandler& g) {
  stream_msg_splitter<ControlHandler, DataHandler> h{x, f, g};
  apply_visitor(h, x.content);
}
...
public:
  stream_serv_class(actor_config& cfg, actor basp, fixture& parent)
      : raw_event_based_actor(cfg),
        basp_(std::move(basp)),
        fixture_(parent),
        ssu_(this, *this, basp_),
        ssd_(this, *this, basp_) {
    // nop
  }
  ...
  behavior make_behavior() override {
    return {
      [=](stream_msg& x) -> delegated<message> {
        ...
        if (ssd_.has_stream(x.sid)) {
          // This stream is inbound.
          ssd_(x);
        } else {
          // This stream is outbound.
          ssu_(x);
        }
        //split_stream_msg(x, ssd_, ssu_);
        return {};
      },
      ...
      [=](sys_atom, ok_atom, int32_t credit) {
        ...
        if (cme->sender != nullptr) {
          ssd_.add_credit(cme->sender->node(), credit);
        } else {
...
private:
  actor basp_;
  fixture& fixture_;
  detail::incoming_stream_multiplexer ssu_;
  detail::outgoing_stream_multiplexer ssd_;
};
...
struct fixture : test_coordinator_fixture<> {
  ...
  void kill_em_all() {
    for (auto x : {basp, streamer, sum_up, sum_up_proxy,
                   stream_serv1, stream_serv2})
...
CAF_TEST(stream_interception) {
  ...
  // stream_serv1 [via BASP] --(stream_msg::open)--> stream_serv2
  expect_network_traffic(stream_serv1, stream_serv2);
  expect((stream_msg::open),
         from(stream_serv1).to(stream_serv2).with(_, stream_serv1, _, _, false));
  ...
  expect((stream_msg::open),
         from(stream_serv2).to(sum_up).with(_, stream_serv2, _, _, false));
